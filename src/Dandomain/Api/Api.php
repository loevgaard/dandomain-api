...
class Api {
    ...
    public function getOrdersInModifiedInterval(\DateTime $dateStart, \DateTime $dateEnd) {
        $this->query = "/admin/WEBAPI/Endpoints/v1_0/OrderService/{$this->apiKey}/GetByModifiedInterval?start=" . $dateStart->format('Y-m-dTH:i:s') . "&end=" . $dateEnd->format('Y-m-dTH:i:s');
        return $this->run();
    }
