<?php
namespace Loevgaard\Dandomain\Api\Request\Customer;

use Assert\Assert;
use Loevgaard\Dandomain\Api\Request\ObjectRequest;
use Loevgaard\Dandomain\Api\Request\RequestInterface;

abstract class CreateDiscountRequest extends ObjectRequest
{
    /**
     * @var string
     */
    protected $type;

    public function __construct(array $discount, string $type)
    {
        Assert::that($discount)->notEmpty();
        Assert::that($type)->choice(DiscountType::getTypes());

        $this->type = $type;

        parent::__construct(RequestInterface::METHOD_POST, sprintf('/admin/WEBAPI/Endpoints/v1_0/DiscountService/{KEY}/%s', $this->type), $discount);
    }

    /**
     * @return string
     */
    public function getType(): string
    {
        return $this->type;
    }
}
